const prompt = require("prompt-sync")(); // to Scan

const ROWS = 3;
const COLS = 3;

const SYMBOLS_COUNT ={
    A:2,
    B:4,
    C:6,
    D:8
};

const SYMBOLS_VALUES= {
    A:5,
    B:4,
    C:3,
    D:2
};

const deposit = () => {
    while(true){
        const depositAmt = prompt("Enter the amount to deposit: ");
        const numOfdeposit = parseInt(depositAmt); // by defsult its string so we change that to int or float

        if(isNaN(numOfdeposit) || numOfdeposit<=0 ){
            console.log("Invalid deposit try again");
        }else{
            return numOfdeposit;
        }
    }
};

const getNoofLines = () =>{
    while(true){
        const getLines = prompt("Enter no.of lines to bet on(1-3): ");
        const getNumber = parseInt(getLines);

        if(isNaN(getNumber) || getNumber <=0 || getNumber > 3){
            console.log("Invalid Bet lines try again");
        }else{
            return getNumber;
        }
    }
};

const getBetAmount = (balance , lines) =>{
  while(true){
    const getBet = prompt("Enter the amount to bet amount per line: ");
    const checkBet = parseInt(getBet);

    if(isNaN(checkBet) ||  checkBet * lines > balance || checkBet<0){
        console.log("Invalid bet amount try again");
    }else{
        return checkBet;
    }
  }
};

const spin = () =>{
    const symbols = [];
    for(const [symbol , count] of Object.entries(SYMBOLS_COUNT)){
        for(let i =0;i<count;i++){
            symbols.push(symbol);
        }
       
    }

    const reels = [[],[],[]];
    for(let i =0;i<COLS;i++){
        reels.push([]);
        const reelSymbols = [...symbols];
        for(let j =0;j<ROWS;j++){
            const randomIndex = Math.floor(Math.random() * reelSymbols.length);
            const selectedSymbol = reelSymbols[randomIndex];
            reels[i].push(selectedSymbol);
            reelSymbols.splice(randomIndex,1); 
        }
    }

    return reels;
};

const transpose = (reels) =>{
    const rows = [];

    for(let i =0;i<ROWS;i++){
        rows.push([]);
        for(let j =0;j<COLS;j++){
            rows[i].push(reels[j][i]);
        }
    }

    return rows;
};

const printRows = (rows) =>{
    for(const row of rows){
        let rowString = "";
        for(const [i,symbol] of row.entries()){
            rowString += symbol;
            if(i != row.length -1){
                rowString += " | ";
            }
        }
        console.log(rowString);
    }
};

const getWinnings = (rows , bet , lines) => {
    let winning = 0;

    for(let row = 0; row < lines ; row++){
        const symbols = rows[row];
        let allSame = true;

        for(const symbol of symbols){
            if(symbol != symbols[0]){
                allSame = false;
                break;
            }
        }

        if(allSame){
            winning += bet * SYMBOLS_VALUES;
        }
    }

    return winning;
};

const game = () =>{
    let balance = deposit();
    while(true){
        console.log("Your Balance Amount is $"+ balance.toString());
        const betLines = getNoofLines();
        const bet = getBetAmount(balance , betLines);
        balance -= bet * betLines;
        const reels = spin();
        const rows = transpose(reels);
        printRows(rows);
        const winnings = getWinnings(rows, bet , betLines);
        balance += winnings;
        console.log("You won, $" + winnings.toString());

        if(balance<=0){
            console.log("You ran out of money! Recharge again");
            break;
        }

        const playAgain = prompt("Do you wanna play again (y/n) ?");

        if(playAgain != 'y') break;
    }
    
};

game();


